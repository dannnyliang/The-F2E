...
class TransactionItem extends React.Component {
  ...
  render () {
    const amount = this.props.amount.replace(/(\d{1,3})(?=(\d{3})+$)/g, function ($1) {
      return $1 = $1 + ','
    })
    return (
      <Row className="trans-item d-flex justify-content-between align-items-center">
        ...
        <Col md='2' className={`p-0 text-right ${this.props.arrow.iconName}`}>
          ...
          <span id={this.props.id} onMouseEnter={this.toggle} onMouseLeave={this.toggle}>{this.props.percent}</span>
          <Popover className="bg-dark" placement="top" isOpen={this.state.popoverOpen} target={this.props.id} toggle={this.toggle}>
